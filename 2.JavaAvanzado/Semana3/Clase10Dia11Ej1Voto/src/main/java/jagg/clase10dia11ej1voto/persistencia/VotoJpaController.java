 ...
 * @author JUNAN
 */
public class VotoJpaController {
    public VotoJpaController() {
        emf = Persistence.createEntityManagerFactory("votoUP");
    }
    ...
    public void edit(Voto voto) throws NonexistentEntityException, Exception {
        ...
        try {
            ...
            if (msg == null || msg.length() == 0) {
                Long id = voto.getId();
                if (findVoto(id) == null) {
                    throw new NonexistentEntityException("The voto con id  " + id + " no longer exists.");
                }
            }
            throw ex;
        } finally {
